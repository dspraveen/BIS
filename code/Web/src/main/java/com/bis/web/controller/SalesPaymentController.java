...
public class SalesPaymentController {
    ...
    public ModelAndView transactionsBetween(@RequestParam(value = "fromDate", required = true) Date fromDate, @RequestParam(value = "toDate", required = true) Date toDate, @RequestParam(value = "hawkerId", required = false, defaultValue = "-1") int hawkerId) {
        ...
        List<PaymentHistorySales> salesPayments;
        if (hawkerId < 1) {
            salesPayments = salesPaymentService.getSalesPayments(fromDate, toDate);
        } else {
            salesPayments = salesPaymentService.getSalesPayments(hawkerMasterService.get(hawkerId), fromDate, toDate);
        }
